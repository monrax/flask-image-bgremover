...
curl -H "Content-type:application/json" -d @input.json http://$HOST:8080/v1/remove | jq -r '.result|ltrimstr("data:image/png;base64,")' | base64 -d > images/output-v1.png

jq '.expr = "convert(image,'"'L'"')"' input.json > v2.json && mv v2.json input.json

echo "\nTesting /v2/remove endpoint..."
curl -H "Content-type:application/json" -d @input.json http://$HOST:8080/v2/remove | jq -r '.result|ltrimstr("data:image/png;base64,")' | base64 -d > images/output-v2.png
