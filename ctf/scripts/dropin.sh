...
curl -H "Content-type:application/json" -d @input.json http://localhost:8080/v2/remove | jq -r '.result|ltrimstr("data:image/png;base64,")' | base64 -d > images/output-exploit.png
